...
Jalankan script ini untuk menambahkan data mahasiswa ke database.
...
def seed():
    ...
    with get_connection() as conn:
        ...
        for nim, nama, prodi, angkatan in MAHASISWA:
            ...
            if existing:
                ...
                conn.execute(
                    "INSERT INTO mahasiswa (nim, nama, prodi, angkatan, status) VALUES (?, ?, ?, ?, 'aktif')",
    ...
    with get_connection() as conn:
        rows = conn.execute("SELECT nim, nama, prodi, angkatan FROM mahasiswa ORDER BY nim").fetchall()
